...
bool LazyCliffordRoutingMethod::update_from_passed_operations(
    MappingFrontierPtr& mapping_frontier, const ArchitecturePtr& architecture){
      ...
      if(this->tableau_qubits_.size() > 0){
        for(const MappingFrontier::Operation& operation : mapping_frontier->passed_operations){
          ...
          if(type != OpType::Barrier){
            ...
            switch (type) {
              ...
              case OpType::Rx: {
                for (const LazySynthesisTableau& t : this->tableau_) {
                  if (t->dependencies_.count(operation.nodes[0]) != 0) {
                    t->pauli_gadgets_.push_back(t->extract_qubit_pauli_tensor(operation.nodes[0], operation.op));
                    modified = true;
                    break;
                  }
                }
              ...
              case OpType::CZ: {
                ...
                if (kt == this->tableau_.end()) {
                  ...
                  if (lt != this->tableau_.end()) {
                    ...
                    lt->add_qubit(q0); 
                    kt->update_gadgets({{qubit0, qubit1}, operation.op, operation.vertex});
                  }
                } else {
...
bool LazyCliffordRoutingMethod::update_from_mapping_frontier(
  ...
  for (auto it = mapping_frontier_->linear_boundary->get<TagKey>().begin();
    ...
    // We update given tableau by looking at "passed operations" after advancing frontier
    // We make new tableau by looking at gates at frontier boundary
    // We manually move the boundary past a gate if it's added
    if(type == OpType::CX || type == OpType::CZ){
        ...
        while (jt != mapping_frontier_->linear_boundary->get<TagKey>().end()) {
          ...
          if (v0 == v1) {
            ...
            auto update_2q_gadget =
                [&kt, &op, &qubit0, &qubit1, &port0, &port1 ]() {
                  // TODO: Doesn't matter for CZ case, though there might be logic for improving
                  // still keep this way for now as easy
                  // => qubit0 is control
                  if (port0 < port1) {
                    kt->update_gadgets({{qubit0, qubit1}, operation.op, operation.vertex});
                  }
                  // => qubit1 is control
                    ...
                    kt->update_gadgets({{qubit1, qubit0}, operation.op, operation.vertex});
                  }
                };
            ...
            if (kt == this->tableau_.end()) {
              ...
              if (lt == this->tableau_.end()) {
                ...
                replacement_edges.insert({qubit0, this->mapping_frontier_->circuit_.get_next_edge(v0, e0)});
                replacement_edges.insert({qubit1, this->mapping_frontier_->circuit_.get_next_edge(v1, e1)});
